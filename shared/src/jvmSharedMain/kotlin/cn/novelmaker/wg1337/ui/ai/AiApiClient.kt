...
class AiApiClient(
    ...
    fun sendChatRequestStream(
        ...
        Thread {
            try {
                ...
                AppLogger.d(TAG, "流式请求 -> $chatEndpoint | 模型: $model | 消息数: ${messages.size} | 工具: ${tools?.length() ?: 0}")
                ...
                AppLogger.d(TAG, "流式响应码: $responseCode")
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                        ...
                        if (currentLine.startsWith("data: ")) {
                            ...
                            try {
                                ...
                                if (errObj != null) {
                                    ...
                                    AppLogger.e(TAG, "流式API返回错误: type=$errType, message=$errMsg")
                                    AppLogger.e(TAG, "完整错误响应: $data")
                                    apiError = errMsg
                                    break
                                }
                                ...
                                AppLogger.w(TAG, "流式数据解析异常，跳过该chunk: ${e.message}")
                                AppLogger.w(TAG, "异常chunk内容: $data")
                            }
                        }
                    }
                    reader.close()
                    ...
                    AppLogger.e(TAG, "流式请求失败: HTTP $responseCode")
                    AppLogger.e(TAG, "错误响应体: $err")
                    connection.disconnect()
                    onComplete(false, "API请求失败 ($responseCode): $err")
                }
            } catch (e: Exception) {
                AppLogger.e(TAG, "流式请求网络异常", e)
    ...
    fun sendChatRequest(
        ...
        AppLogger.d(TAG, "非流式请求 -> $chatEndpoint | 模型: $model | 消息数: ${messages.size} | 工具: ${tools?.length() ?: 0}")
        return try {
            val url = URL(chatEndpoint)
            val connection = url.openConnection() as HttpURLConnection
            connection.apply {
            ...
            AppLogger.d(TAG, "非流式响应码: $respCode")
            ...
            if (respCode == HttpURLConnection.HTTP_OK) {
                ...
                if (errObj != null) {
                    val errMsg = errObj.optString("message", "API错误")
                    val errType = errObj.optString("type", "")
                    AppLogger.e(TAG, "非流式API业务错误: type=$errType, message=$errMsg")
                    AppLogger.e(TAG, "完整错误响应: $response")
                ...
                if (choices != null && choices.length() > 0) {
                    ...
                    AppLogger.e(TAG, "非流式API返回格式异常: choices为空或格式错误")
                    AppLogger.e(TAG, "完整响应: $response")
                    ApiResult(false, errorMessage = "API返回格式异常")
                }
            } else {
                val err = connection.errorStream?.let { BufferedReader(InputStreamReader(it)).readText() } ?: "未知错误"
                AppLogger.e(TAG, "非流式请求失败: HTTP $respCode")
                AppLogger.e(TAG, "错误响应体: $err")
                connection.disconnect()
                ApiResult(false, errorMessage = "API请求失败 ($respCode): $err")
            }
        } catch (e: Exception) {
            AppLogger.e(TAG, "非流式请求网络异常", e)
