...
object BackupManager {
    ...
    /** 备份单个项目 */
    fun backup(project: Project, password: String, signature: String, outputFile: File): Boolean {
        val tmpDir = newTempDir("backup_")
    ...
    /** 备份多个项目到单个文件 */
    fun backupProjects(projects: List<Project>, password: String, signature: String, outputFile: File): Boolean {
        if (projects.isEmpty()) return false
        if (projects.size == 1) return backup(projects.first(), password, signature, outputFile)
        val tmpDir = newTempDir("multiback_")
        return try {
            projects.forEach { p ->
    ...
    fun restore(backupFile: File, password: String): List<String> {
        val tmpDir = newTempDir("restore_")
        try {
            val zipFile = File(tmpDir.parentFile, "${tmpDir.name}.zip")
    ...
    private fun restoreProjectDir(dir: File, restored: MutableList<String>) {
        ...
        try {
            ...
            if (aiDir.exists()) {
                val targetAiDir = File(Platform.internalDataDir(), "ai_chats"); targetAiDir.mkdirs()
                aiDir.listFiles()?.forEach { f -> f.copyTo(File(targetAiDir, f.name), overwrite = true) }
            }
            val repo = cn.novelmaker.wg1337.data.repository.ProjectRepository()
    ...
    private fun copyAiData(tmpDir: File, projectId: String) {
        val aiDir = File(Platform.internalDataDir(), "ai_chats")
    ...
    private fun newTempDir(prefix: String): File =
        File(System.getProperty("java.io.tmpdir"), "$prefix${System.currentTimeMillis()}")
            .apply { mkdirs() }
